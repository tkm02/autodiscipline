...
export const objectifService = {
  ...
  updateProgression: async (objectifId, date, valeur) => {
    const getToken = () => localStorage.getItem("token")
    const response = await fetch(`${API_URL}/objectifs/${objectifId}/progression`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
  ...
  // Mettre à jour les progressions manquantes
  updateMissingProgressions: async () => {
    const getToken = () => localStorage.getItem("token")
    const response = await fetch(`${API_URL}/objectifs/update-missing-progressions`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${getToken()}`,
    ...
    if (!response.ok) {
      throw new Error("Erreur lors de la mise à jour des progressions manquantes")
    }
